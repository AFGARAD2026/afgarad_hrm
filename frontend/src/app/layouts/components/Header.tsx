...
import { 
  Menu, Search, X, Calendar, Zap, Users, CalendarClock, Building2, 
  BarChart3, Receipt, Sun, Moon, Bell, CheckCircle2, UserCircle, Settings, LogOut 
} from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const filteredEmployeesForSearch = searchQuery.trim() === '' ? [] : employees.filter(e => 
    e.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    e.role.toLowerCase().includes(searchQuery.toLowerCase()) || 
    e.department.toLowerCase().includes(searchQuery.toLowerCase())
  ).slice(0, 5);

  const filteredDeptsForSearch = searchQuery.trim() === '' ? [] : departments.filter(d => 
  ...
  return (
    <header className={`sticky top-0 ${isDarkMode ? 'bg-slate-900/95 border-slate-800 text-white' : 'bg-white border-slate-100 text-slate-955'} border-b z-35 px-4 lg:px-6 h-15 flex items-center justify-between shadow-xs shrink-0 transition-colors duration-300`}>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 select-none">
          ...
          <div>
            <p className={`text-xs font-black tracking-tight ${isDarkMode ? 'text-white' : 'text-slate-900'} leading-none`}>Enterprise HRMS</p>
            <p className="text-[10px] text-slate-400 font-semibold leading-normal mt-0.5">SaaS Administration Node</p>
          </div>
        </div>
      </div>
      ...
      <div className="hidden md:block relative w-64 lg:w-96 mx-4">
        <div className="relative">
          ...
          {searchQuery && (
            <button 
              onClick={() => { setSearchQuery(''); setSearchResultsOpen(false); }}
        ...
        <AnimatePresence>
          {searchResultsOpen && searchQuery.trim() !== '' && (
            <>
              <div className="fixed inset-0 z-40" onClick={() => setSearchResultsOpen(false)}></div>
              <motion.div
                initial={{ opacity: 0, scale: 0.98, y: 5 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.98, y: 5 }}
                className={`absolute left-0 mt-1.5 w-full ${
                  isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100 shadow-slate-950/80' : 'bg-white border-slate-200 text-slate-900 shadow-xl'
                } border rounded-xl overflow-hidden shadow-2xl z-50`}
              >
                {/* Employees section */}
                <div className="p-2 px-2.5 space-y-1">
                  <p className="text-[9px] uppercase font-bold text-slate-400 px-2 tracking-wider">Matched Personnel</p>
                  {filteredEmployeesForSearch.length === 0 ? (
                    <p className="text-[10px] text-slate-400 px-2 py-1.5 font-medium italic">No personnel found</p>
                  ) : (
                    filteredEmployeesForSearch.map(emp => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          toast.success(`Viewing ${emp.name} profile in Directory`);
                        }}
                        className={`w-full flex items-center justify-between text-left px-2 py-1.5 rounded-lg text-xs ${
                          isDarkMode ? 'hover:bg-slate-800 text-slate-300' : 'hover:bg-slate-50 text-slate-705'
                        } transition-all`}
                      >
                        ...
                        <span className={`text-[9.5px] px-1.5 py-0.5 rounded ${
                          isDarkMode ? 'bg-slate-805 text-slate-400' : 'bg-slate-100 text-slate-550'
                ...
                {/* Departments section */}
                <div className="p-2 px-2.5 space-y-1">
                  <p className="text-[9px] uppercase font-bold text-slate-400 px-2 tracking-wider">Corporate Units</p>
                  {filteredDeptsForSearch.length === 0 ? (
                    <p className="text-[10px] text-slate-400 px-2 py-1.5 font-medium italic">No departments matched</p>
                  ) : (
                    filteredDeptsForSearch.map(d => (
                      ...
                      >
                        ...
                        <span className={`text-[9.5px] px-1.5 py-0.5 rounded font-mono ${
                          isDarkMode ? 'bg-slate-800 text-teal-400' : 'bg-teal-50 text-teal-750'
      ...
      <div className={`hidden lg:flex items-center gap-1.5 ${isDarkMode ? 'text-slate-400 bg-slate-800/60 border-slate-800' : 'text-slate-500 bg-slate-50 border-slate-100'} border px-3 py-1.5 rounded-xl`}>
        <Calendar size={12} className={isDarkMode ? 'text-slate-500' : 'text-slate-400'} />
      ...
      <div className="flex items-center gap-2.5 lg:gap-3.5">
              ...
              <>
                ...
                <motion.div
                  ...
                  className={`absolute right-0 mt-2 w-56 ${
                    isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100 shadow-slate-950/80' : 'bg-white border-slate-150 text-slate-950 shadow-xl'
                  } border rounded-xl p-2 z-50 space-y-1`}
                >
                  <div className="pb-1.5 px-2 border-b border-slate-800/60 mb-1.5">
                    <p className="text-[9px] uppercase font-bold text-slate-400 tracking-wider">Fast Command Launcher</p>
                  </div>
                  <button
                    onClick={() => {
                      setQuickActionsOpen(false);
                      setActiveView('Directory');
                      if (setIsAddEmployeeOpenFromDashboard) setIsAddEmployeeOpenFromDashboard(true);
                    ...
                    className={`w-full flex items-center gap-2.5 text-left px-2 py-2 rounded-lg text-xs ${
                      isDarkMode ? 'hover:bg-slate-800 hover:text-white text-slate-300' : 'hover:bg-slate-50 hover:text-slate-900 text-slate-705'
                  ...
                  <button
                    onClick={() => {
                      setQuickActionsOpen(false);
                      setActiveView('Leave');
                      if (setIsAddLeaveOpenFromDashboard) setIsAddLeaveOpenFromDashboard(true);
                  ...
                  <button
                    onClick={() => {
                      setQuickActionsOpen(false);
                      setActiveView('Departments');
                      if (setIsAddDeptOpenFromSidebar) setIsAddDeptOpenFromSidebar(true);
                      toast.info("Ready to append department");
                    }}
                    className={`w-full flex items-center gap-2.5 text-left px-2 py-2 rounded-lg text-xs ${
                      isDarkMode ? 'hover:bg-slate-800 hover:text-white text-slate-305' : 'hover:bg-slate-50 hover:text-slate-900 text-slate-705'
                    } font-semibold transition-all cursor-pointer`}
                  >
        ...
        <button
          onClick={() => {
            ...
            toast.info(`Switched to ${!isDarkMode ? 'Cool Slate Dark' : 'Standard Light'} mode`);
          }}
              ...
              <>
                ...
                >
                  <div className="flex items-center justify-between pb-1 border-b border-slate-800/40">
                    <h4 className="text-2xs font-bold uppercase tracking-wider">Awaiting Operations</h4>
                    <span className="text-[9px] bg-indigo-650 text-indigo-100 px-1.5 py-0.5 rounded-md font-extrabold">{notificationsCount} tasks</span>
                  </div>
                  ...
                  <div className="divide-y divide-slate-800/40 max-h-56 overflow-y-auto space-y-2">
                    {leaveRequests.filter(l => l.status === 'Pending').length === 0 ? (
                      <div className="py-6 text-center text-slate-400">
                        ...
                        <p className="text-[10px] font-semibold">Operational Compliance OK</p>
                        <p className="text-[9px] mt-0.5 text-slate-500">No administrative signals waiting.</p>
                      </div>
                    ) : (
                      leaveRequests
                        .filter(l => l.status === 'Pending')
                        .map((leave) => (
                          <div key={leave.id} className="pt-2 first:pt-0 space-y-1 text-left">
                            <p className="text-[10.5px] font-bold">{leave.employeeName}</p>
                            <p className="text-[9.5px] text-slate-450 leading-normal">Filed "{leave.type}" absence request for {leave.days} {leave.days === 1 ? 'day' : 'days'}.</p>
                            <button 
                              onClick={() => {
                                setActiveView('Leave');
        ...
        <span className={`w-px h-6 ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`}></span>
        ...
        <div className="relative">
          ...
          >
            <img 
              ...
              alt="Amara Diop" 
              className={`w-8 h-8 rounded-full border ${isDarkMode ? 'border-slate-805' : 'border-slate-205'} object-cover`}
              referrerPolicy="no-referrer"
            />
            <div className="hidden lg:block text-left space-y-0.5">
              <p className="text-xs font-bold leading-none">Amara Diop</p>
              <p className="text-[10px] text-slate-400 font-semibold leading-normal">Corp HR Director</p>
            </div>
          </button>
          ...
          <AnimatePresence>
            {profileDropdownOpen && (
              <>
                <div className="fixed inset-0 z-40 bg-transparent" onClick={() => setProfileDropdownOpen(false)}></div>
                <motion.div
                  initial={{ opacity: 0, scale: 0.95, y: 10 }}
                  animate={{ opacity: 1, scale: 1, y: 0 }}
                  exit={{ opacity: 0, scale: 0.95, y: 10 }}
                  className={`absolute right-0 mt-2 w-64 ${
                    isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100 shadow-slate-950/85' : 'bg-white border-slate-150 text-slate-950 shadow-xl'
                ...
                >
                  ...
                  <div className="flex items-center gap-3 p-1 text-left">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-black truncate">Amara Diop</p>
                      <p className="text-[10px] text-slate-400 font-semibold truncate leading-none mt-0.5">Corp HR Director</p>
                      <p className="text-[9px] text-slate-500 font-medium truncate font-mono mt-1">mohamein3144@gmail.com</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <button
                      onClick={() => {
                        ...
                        toast.success("Amara Diop's comprehensive profile overview requested (compliance node: secured)");
                      }}
                      className={`w-full flex items-center gap-2.5 text-left px-2 py-1.5 rounded-lg text-xs ${
                        isDarkMode ? 'hover:bg-slate-805 text-slate-300' : 'hover:bg-slate-50 text-slate-705'
                      } font-semibold transition-all cursor-pointer`}
                    >
                      <UserCircle size={14} className="text-slate-400 shrink-0" />
                  ...
                  <button
                    onClick={() => {
                      setProfileDropdownOpen(false);
                      toast.warning("Compliance session ended. Re-authenticating system...");
                    }}
