...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => localStorage.getItem('hrms_dark_mode') === 'true');
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => localStorage.getItem('hrms_sidebar_collapsed') === 'true');
  ...
  const getAccentBgClass = () => {
    switch (accentColor) {
      case 'emerald': return 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-100';
      case 'rose': return 'bg-rose-600 hover:bg-rose-500 text-white shadow-rose-100';
      case 'amber': return 'bg-amber-600 hover:bg-amber-500 text-white shadow-amber-100';
      case 'purple': return 'bg-purple-600 hover:bg-purple-500 text-white shadow-purple-100';
      default: return 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-100';
    }
  };

  const getAccentSidebarClass = (itemActive: boolean) => {
    if (!itemActive) return 'text-slate-400 hover:text-white hover:bg-slate-800/60 font-medium';
    switch (accentColor) {
      case 'emerald': return 'bg-emerald-600 text-white shadow-sm font-bold';
      case 'rose': return 'bg-rose-600 text-white shadow-sm font-bold';
      case 'amber': return 'bg-amber-600 text-white shadow-sm font-bold';
      case 'purple': return 'bg-purple-600 text-white shadow-sm font-bold';
      default: return 'bg-indigo-600 text-white shadow-sm font-bold';
    }
  };

  const getAccentSubmenuClass = (isActive: boolean) => {
    if (!isActive) return 'text-slate-400 hover:text-white hover:bg-slate-800/40 font-medium';
    switch (accentColor) {
      case 'emerald': return 'text-emerald-400 font-bold bg-emerald-500/10 border-l-2 border-emerald-500';
      case 'rose': return 'text-rose-400 font-bold bg-rose-500/10 border-l-2 border-rose-500';
      case 'amber': return 'text-amber-400 font-bold bg-amber-500/10 border-l-2 border-amber-500';
      case 'purple': return 'text-purple-400 font-bold bg-purple-500/10 border-l-2 border-purple-500';
      default: return 'text-indigo-400 font-bold bg-indigo-500/10 border-l-2 border-indigo-500';
    }
  };
  ...
  const sidebarMenu: SidebarItem[] = [
    ...
    },
    {
      ...
      subItems: [
        {
          name: 'Records Matrix',
          active: activeView === 'Attendance' && (setAttendanceSubView ? true : false),
          onClick: () => {
        ...
        },
        {
          ...
          active: activeView === 'Attendance' && (setAttendanceSubView ? false : false),
          onClick: () => {
            setActiveView('Attendance');
    ...
    },
    {
      ...
      subItems: [
        {
          name: 'Dashboard',
          active: activeView === 'Payroll' && (setPayrollSubView ? true : false),
          onClick: () => {
        ...
        {
          name: 'Payslips',
          active: activeView === 'Payroll' && (setPayrollSubView ? false : false),
          onClick: () => {
            setActiveView('Payroll');
  ...
  const notificationsCount = leaveRequests.filter(l => l.status === 'Pending').length;

  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-slate-950 text-slate-100 dark' : 'bg-slate-50/40 text-slate-900'} flex flex-col font-sans select-none antialiased transition-colors duration-300`}>
