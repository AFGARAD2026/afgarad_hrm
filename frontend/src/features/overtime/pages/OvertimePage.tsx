  ...
  return (
    <div className="space-y-6 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">Overtime Tracking System</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-805 rounded-xl p-5 shadow-sm flex items-center justify-between">
          <div className="space-y-1 text-left">
            <span className="text-slate-400 text-[10px] uppercase font-bold tracking-wider">Total Excessive Hours</span>
            <p className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">{totalOvertimeHours} Hrs</p>
            <p className="text-[10px] text-slate-500 font-medium">Aggregated excess periods log</p>
          </div>
          <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-950/25 text-amber-600 dark:text-amber-400 border border-amber-100/30 flex items-center justify-center shrink-0">
            <Hourglass size={18} />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-805 rounded-xl p-5 shadow-sm flex items-center justify-between">
          <div className="space-y-1 text-left">
            ...
            <p className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">${totalOvertimeCost.toLocaleString()}</p>
            <p className="text-[10px] text-slate-500 font-medium font-semibold">Accumulated premium payments</p>
          </div>
          <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-950/25 text-indigo-600 dark:text-indigo-400 border border-indigo-100/30 flex items-center justify-center shrink-0">
            <DollarSign size={18} />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-805 rounded-xl p-5 shadow-sm flex items-center justify-between">
          <div className="space-y-1 text-left">
            ...
            <p className="text-2xl font-black text-slate-850 dark:text-white tracking-tight">${averageHourlyRate}/Hr</p>
            <p className="text-[10px] text-slate-500 font-medium">Industry operational benchmark</p>
          </div>
          <div className="w-10 h-10 rounded-xl bg-violet-50 dark:bg-violet-955/25 text-violet-600 dark:text-violet-400 border border-violet-100/30 flex items-center justify-center shrink-0">
            <TrendingUp size={18} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm flex flex-col justify-between overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-150 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50 dark:bg-slate-805/30">
          ...
          <div className="text-xs text-slate-500 font-mono font-semibold hidden sm:block">
              ...
              {filteredOvertimes.length === 0 ? (
                ...
                filteredOvertimes.map((rec) => (
                  <tr key={rec.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20">
                    <td className="py-3 px-4 font-bold text-slate-900 dark:text-slate-100">{rec.employeeName}</td>
                    <td className="py-3 px-4 font-mono text-[10.5px] text-slate-500 dark:text-slate-400">{rec.date}</td>
                    <td className="py-3 px-4">
                      <span className="font-bold text-indigo-700 bg-indigo-50 dark:text-indigo-400 dark:bg-indigo-950/30 px-2 py-0.5 rounded text-[11px]">{rec.hours} Hrs</span>
                    </td>
                    <td className="py-3 px-4 font-mono text-slate-600 dark:text-slate-400">${rec.rate}/hr</td>
                    <td className="py-3 px-4 font-bold text-emerald-605 text-sm">${rec.amount.toLocaleString()}</td>
                    <td className="py-3 px-4 text-slate-600 dark:text-slate-400 font-medium italic truncate max-w-xs" title={rec.purpose}>
                      "{rec.purpose}"
                    </td>
                    <td className="py-3 px-4 text-right">
                      <button 
                        onClick={() => handleDelete(rec.id)}
                        className="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-955 border border-slate-100 dark:border-slate-800 text-slate-400 hover:text-rose-600 rounded-lg transition-all cursor-pointer"
                        title="Delete record"
                      >
                        <Trash2 size={13} />
                      </button>
        ...
        {/* Footer info */}
        <div className="p-3 bg-slate-50 dark:bg-slate-850 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-[10px] text-slate-500">
          <span>Enterprise premium calculations</span>
          <span className="font-semibold text-amber-600 dark:text-amber-400">Sum total hours logged: {totalOvertimeHours} Hrs</span>
        </div>
        ...
        {isLogOpen && (
          <div className="fixed inset-0 z-55 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl max-w-md w-full p-6 space-y-4 shadow-2xl relative z-10"
            >
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                ...
                <button onClick={() => setIsLogOpen(false)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-450 rounded-lg cursor-pointer">
                  <X size={15} />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateSubmit} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <textarea 
                    ...
                    className="w-full text-xs p-2.5 border border-slate-200 dark:border-slate-707 bg-transparent dark:text-white rounded-lg focus:outline-hidden focus:ring-2 focus:ring-indigo-500 resize-none"
                  />
                </div>
