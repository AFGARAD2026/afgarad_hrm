      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-left">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl p-5 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-2xl font-bold text-slate-900 dark:text-white">25 Days / yr</p>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl p-5 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-2xl font-bold text-slate-900 dark:text-white">10 Days / yr</p>
            <p className="text-[10px] text-slate-500">Full base Salary paid sick days</p>
          </div>
          <div className="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-950/25 flex items-center justify-center text-emerald-600 dark:text-emerald-400 border border-emerald-100/30">
            <HeartPulse size={20} />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl p-5 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-2xl font-bold text-slate-900 dark:text-white">12 Weeks</p>
            <p className="text-[10px] text-slate-500">Maternity & Paternity package</p>
          </div>
          <div className="w-10 h-10 rounded-xl bg-pink-50 dark:bg-pink-955/25 flex items-center justify-center text-pink-600 dark:text-pink-400 border border-pink-100/30">
            <Baby size={20} />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl p-5 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{activeLeaves} Active</p>
            <p className="text-[10px] text-slate-500">{pendingRequests} Pending validation</p>
          </div>
          <div className="w-10 h-10 rounded-xl bg-orange-50 dark:bg-orange-955/25 flex items-center justify-center text-orange-600 dark:text-orange-400 border border-orange-100/30">
          ...
          {['All', 'Pending', 'Approved', 'Rejected'].map((filt) => (
            <button
              ...
              className={`text-xs font-semibold px-3 py-2 rounded-xl border transition-all cursor-pointer ${
                selectedFilter === filt 
                  ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 border-slate-900 dark:border-white' 
                  : 'bg-white text-slate-600 border-slate-200 dark:bg-transparent dark:text-slate-350 dark:border-slate-750 hover:bg-slate-50 dark:hover:bg-slate-800'
              }`}
            >
              {filt}
            </button>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto text-left">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-55/40 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredRequests.length === 0 ? (
                ...
                filteredRequests.map((req) => {
                  const statusColors = 
                    req.status === 'Approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900/30' :
                    req.status === 'Pending' ? 'bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-955/20 dark:text-amber-400 dark:border-amber-900/30' :
                    'bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:text-slate-450 dark:border-slate-700';
                    ...
                    <tr key={req.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-all">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3 text-left">
                          ...
                          />
                          <div>
                            <p className="text-xs font-bold text-slate-900 dark:text-slate-200">{req.employeeName}</p>
                      ...
                      {/* Type */}
                      <td className="p-4 text-xs font-semibold text-slate-700 dark:text-slate-300">{req.type}</td>

                      {/* Dates */}
                      <td className="p-4 text-xs font-medium text-slate-500 dark:text-slate-400">
                        {req.startDate} <span className="text-slate-300 dark:text-slate-650">to</span> {req.endDate}
                      </td>

                      {/* Total Net Days */}
                      <td className="p-4 text-xs font-extrabold text-slate-800 dark:text-slate-250">
                        {req.days} {req.days === 1 ? 'day' : 'days'}
                      </td>

                      {/* Reasons */}
                      <td className="p-4 text-xs text-slate-600 dark:text-slate-400 italic max-w-xs truncate" title={req.reason}>
                        "{req.reason}"
                      </td>
                        ...
                        {req.status === 'Pending' ? (
                          <div className="flex items-center gap-1.5 justify-end">
                            ...
                            <button 
                              ...
                              className="w-7 h-7 bg-rose-50 dark:bg-rose-955/30 border border-rose-100 dark:border-rose-900/40 hover:bg-rose-100 text-rose-600 rounded-lg flex items-center justify-center cursor-pointer transition-all active:scale-95"
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col h-full text-left">
                <div className="overflow-y-auto flex-1 p-6 space-y-5">
                  <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
                    <h3 className="text-sm font-extrabold text-slate-900 dark:text-white">Request Time-off Validation</h3>
                    <button 
                      type="button"
                      onClick={handleCloseModal}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <textarea 
                        ...
                        {...register('reason')}
                        className="w-full text-xs font-semibold px-3.5 py-2.5 border border-slate-200 dark:border-slate-700 bg-transparent dark:text-white rounded-xl focus:border-indigo-500 focus:outline-hidden bg-slate-50/50"
                      />
                ...
                {/* Foot submit bar */}
                <div className="p-4 bg-slate-50 dark:bg-slate-805 border-t border-slate-100 dark:border-slate-800 flex gap-2">
                  <button 
                    type="button" 
                    onClick={handleCloseModal}
                    className="flex-1 text-xs text-slate-605 hover:text-slate-800 font-bold border border-slate-200 dark:border-slate-700 hover:bg-slate-100 rounded-xl py-3 cursor-pointer bg-white dark:bg-slate-950 transition-all text-center"
                  >
                    Cancel
                  </button>
                  <button 
                    type="submit"
                    className="flex-1 text-xs text-white font-bold bg-indigo-600 hover:bg-indigo-505 rounded-xl py-3 cursor-pointer shadow-indigo-100 shadow-sm hover:scale-101 active:scale-99 transition-all text-center"
                  >
                    Submit Leave Files
                  </button>
                </div>
