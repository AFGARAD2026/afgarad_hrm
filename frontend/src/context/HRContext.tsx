...
export const HRProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Dynamic recalculation of department employee counts on standard employee changes
    ...
    setEmployees((prev) =>
      prev.map((emp) => {
        if (emp.id === id) {
          ...
          // Sync changes to leave request names if employee name changed
          if (updates.name) {
  ...
  const hireCandidate = useCallback((id: string) => {
    ...
    const newEmp: Employee = {
      ...
      startDate: new Date().toISOString().split('T')[0],
      salary: 95000, // Standard template salary
      performanceRating: 5.0, // High starting rating
      avatar: `https://images.unsplash.com/photo-${cand.gender === 'Female' ? '1494790108377-be9c29b29330' : '1507003211169-0a1dd7228f2d'}?w=150`,
      phone: cand.phone,
