  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Payroll Administration</h2>
      ...
      {activeTab === 'dashboard' ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-2xl font-black text-rose-600">${totalPayroll.toLocaleString()}</p>
                <p className="text-[10px] text-slate-500 font-medium">Net disbursed this month</p>
              </div>
              <div className="w-10 h-10 rounded-xl bg-rose-50 text-rose-600 border border-rose-100/30 flex items-center justify-center shrink-0">
                <DollarSign size={18} />
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-2xl font-black text-slate-800">${averageSalary.toLocaleString()}</p>
                <p className="text-[10px] text-slate-500 font-medium">Standard staff package rate</p>
              </div>
              <div className="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-650 border border-indigo-100/30 flex items-center justify-center shrink-0">
                <Briefcase size={18} />
              </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-2xl font-black text-emerald-600">${totalOvertimeCompensation.toLocaleString()}</p>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-2xl font-black text-slate-800">${totalDeductions.toLocaleString()}</p>
                <p className="text-[10px] text-slate-500 font-medium">Aggregated benefits/withholdings</p>
              </div>
              <div className="w-10 h-10 rounded-xl bg-slate-50 text-slate-500 border border-slate-100/30 flex items-center justify-center shrink-0">
                <Sliders size={18} />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4 lg:col-span-2">
              <div>
                <h3 className="font-bold text-slate-900 text-sm">Disbursed Staff Salaries Overview</h3>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
              <div>
                <h3 className="font-bold text-slate-900 text-sm">Settle Quality Ratios</h3>
              ...
              <div className="h-64 flex items-center justify-center relative">
                ...
                <div className="absolute text-center flex flex-col items-center">
                  ...
                  <p className="text-lg font-black text-slate-800 leading-none mt-1">{payrolls.length}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col justify-between overflow-hidden">
          ...
          <div className="p-4 border-b border-slate-150 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50">
            ...
            <div className="text-xs text-slate-550 font-semibold flex items-center gap-1 font-mono">
                  ...
                  filteredPayrolls.map((rec) => {
                    ...
                    return (
                      <tr key={rec.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="py-3 px-4 font-semibold text-slate-700">{rec.month}</td>
                        <td className="py-3 px-4 font-mono font-semibold text-slate-600">${rec.baseSalary.toLocaleString()}</td>
                        <td className="py-3 px-4 text-emerald-600 font-mono font-bold">+${rec.overtime.toLocaleString()}</td>
                        <td className="py-3 px-4 text-rose-500 font-mono font-bold">-${rec.deductions.toLocaleString()}</td>
                        <td className="py-3 px-4 text-slate-900 font-black text-sm">${rec.netSalary.toLocaleString()}</td>
                        <td className="py-3 px-4">
                        ...
                        <td className="py-3 px-4 text-right">
                          <div className="inline-flex items-center gap-1 justify-end w-full">
                            <button
                              ...
                              className="p-1.5 hover:bg-indigo-50 border border-slate-100 text-slate-500 hover:text-indigo-600 rounded-lg transition-all cursor-pointer"
                            >
                              <FileText size={12} />
                            </button>
                            <button 
                              onClick={() => handleDelete(rec.id)}
                              className="p-1.5 hover:bg-rose-50 border border-slate-100/40 text-slate-400 hover:text-rose-600 rounded-lg transition-all cursor-pointer"
                              title="Delete entry"
                            >
                              <Trash2 size={12} />
                            </button>
          ...
          <div className="p-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-500 rounded-b-xl">
            ...
            <span className="font-semibold text-rose-650">Total sum net pay output: ${totalPayroll.toLocaleString()}</span>
          </div>
          ...
          <div className="fixed inset-0 z-55 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <button onClick={() => setIsPayOpen(false)} className="p-1 hover:bg-slate-100 text-slate-450 rounded-lg">
                  <X size={15} />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateSubmit} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <select 
                    ...
                    className="w-full text-xs p-2.5 border border-slate-200 rounded-lg focus:outline-hidden bg-white"
                  >
                ...
                <div className="bg-slate-50 border border-slate-100 p-3 rounded-lg flex items-center justify-between text-slate-700">
                  ...
                  <span className="font-black text-rose-650 text-sm">
        ...
        {selectedPayslip && (
          <div className="fixed inset-0 z-55 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 15 }}
              className="bg-white border border-slate-200 rounded-xl max-w-lg w-full p-6 space-y-6 shadow-2xl relative z-10"
            >
              {/* Receipt Header */}
              <div className="flex items-start justify-between border-b border-slate-100 pb-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-1.5 text-indigo-650 font-bold uppercase tracking-wider text-xs">
                    <Sliders size={14} />
                    <span>Orbital Global Labs</span>
                  </div>
                  <h3 className="text-sm font-black text-slate-900">Official Earnings Statement</h3>
                ...
                <button onClick={() => setSelectedPayslip(null)} className="p-1 hover:bg-slate-100 text-slate-450 rounded-lg cursor-pointer">
                  <X size={15} />
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-xs bg-slate-50 p-3 rounded-lg border border-slate-100">
                <div className="space-y-1">
                  ...
                  <div className="font-extrabold text-slate-800">{selectedPayslip.employeeName}</div>
                ...
                <div className="space-y-1 border-l border-slate-200 pl-4">
                  ...
                  <div className="font-extrabold text-slate-850 flex items-center gap-1 font-sans">
              ...
              <div className="space-y-3">
                <h4 className="text-[9px] uppercase font-black tracking-wider text-slate-400 pb-1 border-b border-slate-100">Statement breakdown</h4>
              ...
              <div className="pt-4 border-t border-slate-150 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-2xl font-black text-rose-650">${selectedPayslip.netSalary.toLocaleString()}</div>
                </div>
                <div className="text-right">
                  <span className="text-[9px] uppercase font-bold px-2 py-0.5 rounded-sm border bg-emerald-50 text-emerald-700 border-emerald-150 font-mono tracking-wider">
              ...
              <div className="flex gap-2 justify-end pt-3 border-t border-slate-100">
                <button
                  ...
                  className="px-4 py-2 hover:bg-slate-50 text-slate-600 border border-slate-200 text-2xs font-bold rounded-lg cursor-pointer flex items-center gap-1"
                >
                  <Printer size={12} />
                  Print Statement
                </button>
